defmodule KickTracker.Kick.PublicKey do
  @moduledoc """
  Kick's webhook signing key, for checking each envelope again: given in
  configuration (`KICK_PUBLIC_KEY`), or fetched from
  `GET <KICK_API_URL>/public/v1/public-key` and kept.

  Reading the key never waits: it lives in `:persistent_term`, so a slow
  or unreachable Kick API can't hold up the consumer. Fetches run in a
  task, never in this process.

  If a signature fails, the key may have changed: `refresh/2` starts a
  fetch, at most once a minute (so a run of bad messages can't turn the
  consumer into a way to hammer Kick's API), and waits for its result for
  ...
  @doc """
  Fetches the key again, unless that was done in the last minute. Returns
  `{:ok, pem}` with the key held once the fetch is done (or at once when
  none is due), or `{:error, :timeout}` if it isn't done within `wait_ms`
  or this process is unavailable.
  """
  @spec refresh(atom(), timeout()) :: {:ok, String.t() | nil} | {:error, :timeout}
  def refresh(name \\ __MODULE__, wait_ms \\ @refresh_wait_ms) do
    {:ok, GenServer.call(name, :refresh, wait_ms)}
  catch
  ...
  def init(opts) do
    ...
    pem = Keyword.get(opts, :pem, kick[:public_key])
    put(name, pem)

    state = %{
      name: name,
      api_url: Keyword.get(opts, :api_url, kick[:api_url]),
  ...
  defp finish(state, result) do
    case result do
      {:ok, pem} -> put(state.name, pem)
      {:error, reason} -> Logger.warning("could not fetch Kick's public key: #{inspect(reason)}")
    end
