...
Limits, stated: this protects collection on one VPS; if the machine goes, the database goes with it, and collectors elsewhere would first need a replicated database (stage 3). A crash takeover is ~20s by design (a live leader whose connection blipped must be able to get its lease back first). A channel process that starts during a database outage without a snapshot starts afresh, so a stream that ended meanwhile may be closed later by its end event rather than by polling.
## Queue
